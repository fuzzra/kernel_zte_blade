	...
	short max_size)
{
	...
	int	ret = 0;
    /** ZTE_MODIFY longchunyan0004 repaired for connecting to hidden 802.11n ap by longchunyan 2011/10/08*/
    uint8 channel;
    /** ZTE_MODIFY end*/
	if (!list) {
...
#endif 
   /** ZTE_MODIFY longchunyan0003 added for  enabling softap confingation by longchunyan 2011/08/08*/
	...
	char *ext)
{
	...
	if ((wrqu->data.length > 4) && (wrqu->data.length < sizeof(extra))) {
		...
		str_ptr = extra;
        /** ZTE_MODIFY longchunyan0001 repaired for bcm4330 b1&b2 by longchunyan 2011/07/30*/
	    #if 0
		if (get_parameter_from_string(&str_ptr,
